...
// AutoWrong: imports for the typo generator and counter
import helium314.keyboard.latin.autowrong.TypoGenerator;
...
public final class InputLogic {
    ...
    public void commitTyped(final SettingsValues settingsValues, final String separatorString) {
        ...
        if (typedWord.length() > 0) {
            ...
            // AutoWrong: invert autocorrect behavior.
            // If the word is valid (correctly spelled), mangle it. If it's already wrong, leave it.
            final boolean wordIsValid = mSuggestedWords.mTypedWordValid;
            final String wordToCommit;
            if (wordIsValid) {
                final String mangled = TypoGenerator.mangle(typedWord);
                wordToCommit = mangled;
    ...
    private void commitCurrentAutoCorrection(final SettingsValues settingsValues,
        ...
        final String typedWord = mWordComposer.getTypedWord();
        // AutoWrong: invert the autocorrect behavior.
        //   - If mTypedWordValid = true  → word IS correctly spelled → MANGLE IT
        //   - If mTypedWordValid = false → word is NOT in dictionary → commit as-is (leave misspellings untouched)
        // We ignore HeliBoard's suggested autocorrection entirely and always commit the typed word
        // (possibly mangled). This achieves the full behavioral inversion.
        final boolean wordIsValid = mSuggestedWords.mTypedWordValid;
        final String stringToCommit;
        if (wordIsValid) {
            stringToCommit = TypoGenerator.mangle(typedWord);
            WordsDestroyedCounter.increment(mLatinIME);
        } else {
            stringToCommit = typedWord; // already misspelled — leave it untouched
        }
